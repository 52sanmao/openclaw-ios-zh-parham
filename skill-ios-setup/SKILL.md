...
```
python3 ~/.openclaw/workspace/orchestrator/skills/ios-setup/scripts/detect_env.py
```
...
```bash
python3 ~/.openclaw/workspace/orchestrator/skills/ios-setup/scripts/deploy_stats.py
```

This will:
1. Check stats_server.py exists in the workspace
...
**Direct install** — add a system crontab entry:
```bash
WORKSPACE=$(python3 -c "import json; d=json.load(open(os.path.expanduser('~/.openclaw/openclaw.json'))); print(d.get('agents',{}).get('defaults',{}).get('workspace','~/.openclaw/workspace'))")
# Then tell user to run:
crontab -e
# Add this line:
@reboot bash /path/to/workspace/orchestrator/scripts/dashboard/ensure_stats_server.sh
```

Use `detect_env.py` output to fill in the correct workspace path automatically.
...
In the iOS app:
1. Open the app
2. Paste the URL and token when prompted
3. Tap Connect
...
If anything failed, clearly say what failed and what they need to do manually.

---

## Notes
- The gateway token comes from: `openclaw config get gateway.auth.token` or from `~/.openclaw/openclaw.json`
- Stats server scripts live at: `~/.openclaw/workspace/orchestrator/scripts/dashboard/`
